@extends('layouts.base', ['title' => 'Verificar correo'])
            ...
            <div class="col-md-auto">
                ...
                <div class="card auth-box-form border-0 mb-0">
                    ...
                    <div class="card-body min-vh-100 d-flex flex-column justify-content-center">
                        ...
                        <div class="mt-auto">
                            <p class="text-muted text-center auth-sub-text mx-auto">Te hemos enviado un código de
                                verificación de 6 dígitos a</p>
                            ...
                            <form class="mt-4" method="POST" action="{{ route('verify.code.check') }}">
                                @csrf

                                <input type="hidden" name="email" value="{{ $email ?? '' }}">

                                <div class="mb-3">
                                    <label class="form-label d-flex">Ingresa tu código de 6 dígitos <span
                                            class="text-danger">*</span></label>
                                    <div class="d-flex gap-2 two-factor">
                                        <input class="form-control text-center" required maxlength="1" type="text"
                                ...
                                <div class="d-grid">
                                    <button class="btn btn-primary fw-semibold py-2" type="submit">Confirmar</button>
                                </div>
                            </form>
                        </div>
                        <p class="text-muted text-center mt-4 mb-0">
                            ...
                            <button id="resend-btn"
                                class="btn btn-link text-decoration-underline link-offset-2 fw-semibold p-0"
                                {{ $remaining <= 0 ? '' : 'disabled' }}>
                                <span id="btn-text">{{ $remaining > 0 ? 'Reenviar (' . $remaining . 's)' : 'Reenviar código' }}</span>
                            </button>
                        </p>

                        <p class="text-muted text-center mt-4 mb-0">
                            Regresar a <a class="text-decoration-underline link-offset-3 fw-semibold"
...
@section('scripts')
    ...
    <script>
        // Script para manejar los inputs de 6 dígitos
        document.addEventListener('DOMContentLoaded', function() {
            const inputs = document.querySelectorAll('.two-factor input[type="text"]');
            const hiddenCode = document.getElementById('verification-code');
    ...
    {{-- Script para manejar el reenvío del código con temporizador de 10 minutos --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function startTimer() {
                timerInstance = setInterval(() => {
                    if (countdown > 0) {
                        countdown--;
                        btnText.textContent = `Reenviar (${formatTime(countdown)})`;
                    } else {
            ...
            // Iniciar timer al cargar la página si countdown > 0
            if (countdown > 0) {
                btnText.textContent = `Reenviar (${formatTime(countdown)})`;
                startTimer();
            }
            ...
            resendBtn.addEventListener('click', function(e) {
                ...
                fetch('{{ route('verify.code.resend') }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        ...
                        Swal.fire({
                            ...
                            confirmButtonColor: '#089df1',
                            timer: 3000,
                            timerProgressBar: true
                        });

                        // Reiniciar contador a 10 minutos (600 segundos)
                        countdown = 600;
                        btnText.textContent = `Reenviar (${formatTime(countdown)})`;

                        // Limpiar timer anterior y comenzar uno nuevo
                    ...
                    .catch(() => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Ocurrió un error al reenviar el código. Por favor, intenta de nuevo.',
